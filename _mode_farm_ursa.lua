-- JUNGLE URSA, CODE IS BASICALLY BLOODSEEKER JUNGLE BY PLATINUM WITH A FEW THINGS EDITED
...
local function CanFarmHardCamp()
	if Utility.GetHeroLevel()>4 then
		return true;
	end
	return false;
end
...
local function CSing()
	...
	local creeps=npcBot:GetNearbyNeutralCreeps(1000);
	
	if creeps==nil or #creeps==0 then
		print("nenhum creep detectado")
		if IsHard then
...
local function HarassEnemy()
	local npcBot=GetBot();
	
	local enemy,health = Utility.GetWeakestHero(1000);
	if not (Utility.NotNilOrDead(enemy)) then
		JunglingState=JunglingStates.MovingToPos;
		return;
	end
...
[JunglingStates.HarassEnemy]=HarassEnemy,
...
function GetDesire()
	...
	if not Utility.IsItemInInventory("item_lifesteal") and npcBot:GetLevel()<6 then
		npcBot.IsJungling=true;
		return 0.16;
	end
